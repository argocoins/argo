 ...
 *
 *   // network
 *   CArgoNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.argo.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CArgoNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
// // can be under: ArgoNetwork
// //   -- signature requirements : Key1(User)
// class CArgoNetworkVariable : public CGovernanceObject
// {
// private:
