...
const char * const BITCOIN_CONF_FILENAME = "argo.conf";
const char * const BITCOIN_PID_FILENAME = "argod.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "argo" is a composite category enabling all Argo-related debug output
            if(ptrCategory->count(string("argo"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ArgoCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ArgoCore
    // Mac: ~/Library/Application Support/ArgoCore
    // Unix: ~/.argocore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ArgoCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ArgoCore";
#else
    // Unix
    return pathRet / ".argocore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty argo.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override argo.conf
